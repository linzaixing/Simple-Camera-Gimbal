...
class YoloUtil:
    ...
    def detect_object(self, image, session, model_inputs, input_width, input_height):
        ...
        output_image, boxes = self.postprocess(result_image, outputs, input_width, input_height, img_width, img_height)
...
def detect_run(frame, detector, session, model_inputs, input_width, input_height):
    # 使用检测模型对读入的帧进行对象检测
    output_image, boxes = detector.detect_object(frame, session, model_inputs, input_width, input_height)
    return output_image, boxes
...
if __name__ == '__main__':
    ...
    if mode == 1:
        ...
        # 使用检测模型对读入的图像进行对象检测
        result_image, boxes = yolo_detector.detect_object(image_data, session, model_inputs, input_width, input_height)
        # 将检测后的图像保存到文件
        ...
        while True:
            ...
            if not ret:
                ...
                break
            # 使用检测模型对读入的帧进行对象检测
            output_image, boxes = yolo_detector.detect_object(frame, session, model_inputs, input_width, input_height)
            # 计算帧速率
            frame_count += 1
            end_time = time.time()
